...
#Caracteres para generar la contraseña / Characters to generate the password
...
def enable():
    ...
    #Alvertencia | Warning
    messagebox.showinfo("Alvertencia" , "Verifique que el Nombre del Archivo no se repita con otro Archivo de Texto por que se sobreescribira")
...
Label_0 = Label(window, text="Programa para Generar Contraselas Aleatorias", background="#3D405B", font=("Arial", 16), fg="#81B29A")
...
Label_1 = Label(window, text="Ingrese el Numero de Caracteres", background="#3D405B" , font=("Arial, 14"), fg="#F4F1DE")
Label_1.grid(row=2, column=0, padx=2, pady=2)
entry_characters = Entry(window, background="#F4F1DE", bd=4, relief="ridge", font=("Arial, 14"), fg="#3D405B")
...
Label_2 = Label(window, text="Guardar la Contraseña en un Archivo .txt", background="#3D405B", font=("Arial", 14), fg="#F4F1DE")
Label_2.grid(row=5, column=0, padx=5, pady=5)

#Guardar las contraseñas en un archivo de texto / Save passwords in a text file
checkbutton_save_passwords = Checkbutton(window, font=("Arial", 10), fg="#F4F1DE", background="#3D405B", text="Guardar las Contraseñas",  variable = enable_save, command = lambda:enable())
checkbutton_save_passwords.grid(row=7, column=0, padx=2, pady=2)

#nombre del archivo de texto / name of the text file
Label_3 = Label(window, text="Nombre del Archivo de Texto", background="#3D405B", font=("Arial", 13), fg="#F4F1DE")
Label_3.grid(row=8, column=0, padx=5, pady=5)
name_txt = Entry(window,background="#F4F1DE", bd=4, relief="ridge", font=("Arial, 14"), fg="#3D405B")
...
button_generate_password = Button(window, text="Generar Contraseña", width=19, height=1, background="#E07A5F", font=("Arial", 13), fg="#F4F1DE", command = lambda:convert_integer())
button_generate_password.grid(row=11, column=0, padx=2, pady=2)

#Mostrar la contraseña generada / Show the generated password
Label_4 = Label(window, text="Contraseña Generada", background="#3D405B", font=("Arial", 14), fg="#F4F1DE")
Label_4.grid(row=12, column=0, padx=2, pady=2)
show_password = Entry(window, background="#F4F1DE", font=("Arial", 14), fg="#3D405B", textvariable = sv)
